...
function ChartView({ rows, type }: { rows: DataRow[]; type: 'bar' | 'line' }) {
  ...
  return <div className="chart-wrap">
    <div className="visualizer-heading"><div><span className="eyebrow">Visual analysis</span><h3>{type === 'bar' ? 'Branch comparison' : 'Activity over time'}</h3></div>{type === 'bar' ? <BarChart3 size={18} /> : <LineChartIcon size={18} />}</div>
