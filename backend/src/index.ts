...
import { containsUnsafeSqlInput, validateQuery } from "./services/queryValidator.js";
...
app.post("/api/chat", (request, response) => {
    ...
    if (containsUnsafeSqlInput(question)) {
        response.status(400).json({ error: "Raw SQL input is blocked. Ask a business question instead." });
        return;
    }
